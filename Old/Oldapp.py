...
class ClassApp:
    def __init__(self) -> None:
        ...
        self.GUI = {
            "EscapeUI" : [
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2-145), (100, 50), "RENAME", scale=self.settings["scale"]["GUI"]), 
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2-85), (100, 50), "OPEN", scale=self.settings["scale"]["GUI"]), 
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2-25), (100, 50), "NEW", scale=self.settings["scale"]["GUI"]),
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2+35), (100, 50), "DELETE", scale=self.settings["scale"]["GUI"]),
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2+95), (100, 50), "Export as PDF", scale=self.settings["scale"]["GUI"]),
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2+155), (100, 50), "SETTINGS", scale=self.settings["scale"]["GUI"])
            ],
            "OPENUI" : [],
            "NEWUI" : [
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2+5), (100, 50), "", scale=self.settings["scale"]["GUI"]), 
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2-45), (100, 50), "Name:", scale=self.settings["scale"]["GUI"])
            ],
            "RENAMEUI" : [
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2+5), (100, 50), "", scale=self.settings["scale"]["GUI"]), 
                OldObjects.ClassButton((self.screen.get_size()[0]/2-50, self.screen.get_size()[1]/2-45), (100, 50), "Name:", scale=self.settings["scale"]["GUI"])
            ...
            "SETTINGSUI" : [
                OldObjects.ClassButtonSlider((self.screen.get_size()[0]/2, self.screen.get_size()[1]/2 - 85), (100, 50), 1, 3)
            ]

        }
        self.GUIRoomFill()
        json = RoomHandler.GetJson(self.settings["RoomFile"]).keys()
        if json:
            self.settings["CurrentRoom"] = next(iter(json))
            self.Room = RoomHandler.ReadRoom(self.settings["RoomFile"], self.settings["CurrentRoom"], self.FONT, self.settings["scale"])
        else:
            RoomHandler.CreateRoom(self.settings["RoomFile"], "Room1")
            self.changeRoom("Room1")
        self.mouse = OldObjects.ClassMouse()
        self.typingMode = [False, None]
        
    def GUIRoomFill(self):
        nr = 0
        self.GUI["OPENUI"] = []
        for room in RoomHandler.GetJson(self.settings["RoomFile"]).keys():
            self.GUI["OPENUI"].append(OldObjects.ClassButton((self.screen.get_size()[0]/2-50, 25*nr+50), (100, 20), room, scale=self.settings["scale"]["GUI"]))
            nr += 1     
    ...
    def saveRoom(self):
        RoomHandler.WriteRoom(self.settings["RoomFile"], self.settings["CurrentRoom"], self.Room, self.settings["scale"])

    def NewRoom(self, ID):
        RoomHandler.CreateRoom(self.settings["RoomFile"], ID)

    def deleteRoom(self, ID):
        RoomHandler.RemoveRoom(self.settings["RoomFile"], ID)
    
    def renameRoom(self, ID, name):
        RoomHandler.RenameRoom(self.settings["RoomFile"], ID, name)
    ...
    def variableUpdate(self):
        ...
        if self.typingMode[0]:
            if type(self.typingMode[1]) == OldObjects.ClassSeat:
    ...
    def create(self, flag:str) -> object:
        match flag:
            case "Seat":
                return OldObjects.ClassSeat((self.mouse.pos[0], self.mouse.pos[1]), self.FONT["Seat"], scale=self.settings["scale"])
            case "Table":
                return OldObjects.ClassTable((self.mouse.pos[0], self.mouse.pos[1]), (100, 100), scale=self.settings["scale"]["table"])
            case "RoundTabel":
                return OldObjects.ClassRoundTable((self.mouse.pos[0], self.mouse.pos[1]), 150, scale=self.settings["scale"]["table"])
            case _:
    ...
    def followCheck(self, table : OldObjects.ClassTable) -> list:
    ...
    def followRoundCheck(self, table : OldObjects.ClassRoundTable) -> list:
        if not pygame.mouse.get_pressed(3)[0]:
            return [None, None, None]
        if self.mouse.holding[0] != None:
            return self.mouse.holding
...
def main() -> int:
    ...
    while app.running:
        ...
        pygame.event.pump()
    app.saveRoom()
    RoomHandler.WriteJson("data/Settings.json", app.settings)
    pygame.quit()
    return 0
